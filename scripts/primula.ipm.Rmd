...
rows <- expand.grid(spring.mean.temp = c(-1.951,0, 0.904),  prev_winter.mean.min.temp = c(-1.949, 0, 0.824), trt = c("control", "drought", "irrigated")) # add this in later
...
for (i in 1:nrow(params)) {
  ...
  params$growth.int[i] <- fixef(gm.st.i)[1] + fixef(gm.st.i)[3] * params$spring.mean.temp[i]
}
...
fixef(pmflow.ti)
#intercept
params$pflow.int <- case_when(params$trt == "control"~  fixef(pmflow.ti)[1] + fixef(pmflow.ti)[3],
                             params$trt == "irrigated"~  fixef(pmflow.ti)[1],
                             params$trt == "drought"~  fixef(pmflow.ti)[1] + fixef(pmflow.ti)[4])
...
for (i in 1:nrow(params)) {
  ...
  params$no.flow.int[i] <- fixef(nf.wt)[1] + fixef(nf.wt)[3] * params$prev_winter.mean.min.temp[i]
}
...
bigmatrix<-function(params_list, mat.size)
  ...
  out_lamb <- data.frame(lambda = lambda(IPMmat), spring.mean.temp = params_list$spring.mean.temp, trt = params_list$trt, prev_winter.mean.min.temp = params_list$prev_winter.mean.min.temp)
...
#     color = ""
#   ) )
(together <- spring.temp+winter+ plot_layout(guides = "collect", axis_titles = "collect") &
  theme(legend.position = "bottom"))

ggsave(here("Figures", "Primula", "primula.determ.pop.growth.png"), plot = together, width = 7.5, height =5)
```
